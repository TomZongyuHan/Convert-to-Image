...
# Main methods included in the pipeline
# Input:
...
def main(filename, isRowCount):
    ...
    # Set all methods need to be run
    ...
    try:
        results = np.load('results/accuracies/results.npy', allow_pickle = True)
    except IOError:
        results = np.zeros((allNum, 4), dtype = np.float64) # allNum should be 2160
    ...
    for normName in normNames:
        ...
        rst = checkSkipMethod('norm', finishNum + normNum - 1, results, [filepath, isRowCount, normName])
        ...
        for drName in drNames:
            ...
            rst = checkSkipMethod('dr', finishNum + drNum - 1, results, [normalizedDataset, drName])
            if isinstance(rst, int):
                finishNum = rst + 1
                continue
            else:
            ...
            for icName in icNames:
                ...
                rst = checkSkipMethod('ic', finishNum + icNum - 1, results, [drResult, icName])
                if isinstance(rst, int):
                    finishNum = rst + 1
                    continue
                else:
                ...
                for CNNName in CNNNames:
                    ...
                    rst = checkSkipMethod('cnn', finishNum, results, [augmentedDataset, CNNName])
                    if isinstance(rst, int):
                        finishNum = rst + 1
                        continue
                    else:
                    ...
                    results[finishNum][0] = results[2]
                    results[finishNum][1] = accuracy[0]
                    results[finishNum][2] = accuracy[1]
                    results[finishNum][3] = accuracy[2]
                    np.save('results/accuracies/results.npy', results)
...
def checkSkipMethod(methodName, finishNum, results, params):
    skipFlag = float(results[finishNum][0]) != float(0)
    if methodName == 'norm':
        if not skipFlag:
...
def saveFinalResult(normNames, drNames, icNames, CNNNames):
    ...
    resultsList = np.array(np.load('results/accuracies/results.npy', allow_pickle = True)).tolist()
    ...
    df.to_csv('results/accuracies/results.csv', index = False)


# Please use the file name that you want to process e.g. yan-rowCount.csv
# Please set True for rowcount dataset, set False for no row count dataset
filename = '.csv'
main(filename, True)
